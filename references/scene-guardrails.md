...
- 中文长文（约 1000 字以上）默认优先考虑 `in-place` scope，尤其是观点文、复盘文、评论文和带段落节奏的公开文本
- 用户 prompt 含 `保长度 / 别缩水 / 字数 / 节奏 / 别删 / 尽量原样` 这类信号时，即使原文没到 1000 字，也优先切到 `in-place`
- `in-place` 下先保字数、句数和段落节奏，只处理句内 AI 味；不要把整段压成短摘要
